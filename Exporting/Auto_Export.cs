...
public class NXJournal
{
    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            // ----------------------------------------------
            //   Map each component to Nx component bodies
            // ----------------------------------------------
    ...
    /// <summary>
    /// Returns TransferItem DTO from component information
    /// </summary>
    /// <returns></returns>
    /// <exception cref="NotImplementedException"></exception>
    private static ComponentDTO ExtractComponentInfo(List<NXOpen.Assemblies.Component> componentList)
    {

        var componentNumber = componentList[0].GetUserAttribute("CO", NXObject.AttributeType.String, -1).StringValue.TrimStart('0');

        var componentName = componentList[0].GetUserAttribute("DES", NXObject.AttributeType.String, -1).StringValue.Replace("\"", "");

        var fileName = componentList[0].Name + '_' + DateTime.Now.ToString("MMddyy") + ".stp";

        var bodies = GetNxBodies(componentList);

        return new ComponentDTO(componentNumber, componentName, bodies.Count, fileName, bodies);

    }

    /// <summary>
    /// Pattern Matching of name
    /// </summary>
    ...
    public static string SerializePayloadToJson(List<ComponentDTO> items, StringBuilder sb = null)
    {
        if (sb == null)
        {
        ...
        sb.Append("{");
        sb.AppendFormat("\"JobNumber\":\"{0}\",", items[0].FileName.Split('_')[0]);
        sb.Append("\"Components\":[");
        for (int i = 0; i < items.Count; i++)
        {
            var item = items[i];
            sb.Append('{');
            sb.AppendFormat("\"ComponentNumber\":{0},", item.ComponentNumber);
            sb.AppendFormat("\"ComponentName\":\"{0}\",", item.ComponentName);
            sb.AppendFormat("\"Quantity\":{0},", item.Quantity);
            sb.AppendFormat("\"FileName\":\"{0}\",", item.FileName);
            sb.AppendFormat("\"CreatedDateTime\":\"{0}\",", item.CreatedDateTime);
            sb.AppendFormat("\"Creator\":\"{0}\"", item.Creator);
            sb.Append('}');
    ...
    /// <summary>
    /// Used as a DTO for the named pipe to DBQuill
    /// </summary>
    public class ComponentDTO 
    {
        ...
        public List<NXObject> Bodies;

        public ComponentDTO(string compNum, string compName, int quantity, string fileName, List<NXObject> bodies)
        {
